          ...
          <div className="mb-4 rounded-2xl border border-border bg-muted/30 p-4">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {templates.map((template) => (
                <Button key={template.id} type="button" size="sm" variant="secondary" disabled={instantiateMutation.isPending} onClick={() => instantiateMutation.mutate(template.id)} className="shrink-0">
                  {template.name}
                </Button>
              ))}
            </div>
