...
export const tasksService = {
  ...
  async createTemplate(data: Pick<TaskTemplate, 'name' | 'title' | 'description' | 'priority' | 'default_duration_days'> & { is_shared?: boolean }) {
    return api.post<TaskTemplate>('/tasks/templates/', data)
  },

  async saveAsTemplate(taskId: number, name: string, is_shared?: boolean) {
    return api.post<TaskTemplate>(`/tasks/${taskId}/save-template/`, { name, is_shared })
  },
